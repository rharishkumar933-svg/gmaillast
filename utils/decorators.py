...
def worker_only(func):
    """Decorator to restrict function to workers or admins"""
    @wraps(func)
    async def wrapper(client: Client, update):
        mongo = client.mongo
        ...
        # Check if user is worker or admin or owner
        from config import OWNER_ID
        is_w = (user_id == OWNER_ID 
                or await mongo.is_admin(user_id) 
                or await mongo.db["workers"].find_one({"user_id": user_id}) is not None)
        
        if is_w:
            return await func(client, update)
        else:
            if isinstance(update, Message):
                await update.reply("❌ This command is restricted to workers!")
            elif isinstance(update, CallbackQuery):
                await update.answer("❌ Worker permission required!", show_alert=True)
