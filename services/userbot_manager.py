...
from config import API_ID, API_HASH, BOT_USERNAME, TARGET_BOT_USERNAME, HOLD_LOG_GROUP_ID, BLOCK_NOTIFICATION_GROUP_ID
...
class UserbotManager:
    ...
    async def _register_event_handlers(self, phone: str, client: TelegramClient):
        ...
        async def check_for_block(message) -> bool:
            text = message.text
            if text and ("blocked for using accounts" in text.lower() or "you are blocked" in text.lower()):
                logger.warning(f"Userbot {phone} was blocked by the target bot! Disconnecting...")
                if BLOCK_NOTIFICATION_GROUP_ID:
                    alert_text = (
                        f"⚠️ **Userbot Blocked Alert!**\n\n"
                        f"📱 **Phone:** `{phone}`\n"
                        f"💬 **Reason/Text:**\n`{text}`\n\n"
                        f"❌ This userbot has been **disconnected** from the system."
                    )
                    try:
                        await self.app.send_message(chat_id=BLOCK_NOTIFICATION_GROUP_ID, text=alert_text)
                    except Exception as ex:
                        logger.error(f"Failed to send block alert to group {BLOCK_NOTIFICATION_GROUP_ID}: {ex}")
                
                await self.disconnect_single_userbot(phone)
                return True
            return False
        ...
        async def on_message_edited(event):
            try:
                if await check_for_block(event.message):
                    return
