...
async def ub_set_reward_callback(client: Client, callback: CallbackQuery):
    ...
    try:
        response = await client.ask(
            chat_id=user_id,
            text="![💰](tg://emoji?id=6030558512252197022) **Set Reward Amount**\n\nSend the reward amount in USD (e.g. `0.15`) or in INR by adding 'inr' after the number (e.g. `12.5 inr`):\n\nSend /cancel to cancel",
            timeout=60
        )
        ...
        val_text = response.text.strip().lower()
        in_inr = False
        if "inr" in val_text or "₹" in val_text:
            in_inr = True
            val_str = val_text.replace("inr", "").replace("₹", "").strip()
        else:
            val_str = val_text
        ...
        try:
            ...
            await response.reply("❌ Invalid value. Please send a valid number.")
            return
        ...
        exchange_rate = await mongo.get_system_setting("exchange_rate", 100.0)
        if in_inr:
            reward_val = val_amount / exchange_rate
        else:
            reward_val = val_amount
